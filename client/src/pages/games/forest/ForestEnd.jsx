// src/pages/games/forest/ForestEnd.jsx
...
const ForestEnd = () => {
    ...
    const successRate = stats && (stats.num_aciertos + stats.num_errores > 0) 
    ...
    return (
        <div className="min-h-[calc(100vh-10rem)] flex flex-col bg-gray-50">
            {/* Header - Mejorada la responsividad */}
            <div className="bg-[#00398A] text-white py-2 px-4 sm:px-6">
                <h1 className="text-lg sm:text-xl font-semibold">Resultados del Juego de Sendero en el Bosque</h1>
            </div>

            {/* Contenido principal - Grid mejorado para dispositivos pequeños */}
            <div className="flex-1 p-2 sm:p-4 grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 overflow-y-auto">
                {/* Columna izquierda */}
                <div className="flex flex-col gap-3 sm:gap-4 h-full">
                    {/* Configuración */}
                    <section className="bg-white rounded-lg shadow p-3 sm:p-4">
                        <h2 className="text-md sm:text-lg font-semibold text-[#00398A] mb-2">
                            Configuración Sesión
                        </h2>
                        <div className="space-y-1 sm:space-y-2">
                            <div className="flex justify-between items-center flex-wrap">
                                <span className="text-sm sm:text-base text-gray-600">Dificultad:</span>
                                <span className="text-sm sm:text-base bg-blue-100 px-2 sm:px-3 py-1 rounded capitalize">
                                    {config?.difficulty || 'No disponible'}
                                </span>
                            </div>
                            <div className="flex justify-between items-center flex-wrap">
                                <span className="text-sm sm:text-base text-gray-600">Nivel inicial:</span>
                                <span className="text-sm sm:text-base bg-blue-100 px-2 sm:px-3 py-1 rounded">
                                    {config?.startingLevel || 1}
                                </span>
                            </div>
                            <div className="flex justify-between items-center flex-wrap">
                                <span className="text-sm sm:text-base text-gray-600">Densidad de objetos:</span>
                                <span className="text-sm sm:text-base bg-blue-100 px-2 sm:px-3 py-1 rounded capitalize">
                                    {config?.objectDensity || 'Normal'}
                                </span>
                            </div>
                            <div className="flex justify-between items-center flex-wrap">
                                <span className="text-sm sm:text-base text-gray-600">Tiempo límite por nivel:</span>
                                <span className="text-sm sm:text-base bg-blue-100 px-2 sm:px-3 py-1 rounded">
                                    {config?.timeLimit ? `${config.timeLimit} segundos` : 'Sin límite'}
                                </span>
                            </div>
                            <div className="flex justify-between items-center flex-wrap">
                                <span className="text-sm sm:text-base text-gray-600">Número de rondas configuradas:</span>
                                <span className="text-sm sm:text-base bg-green-100 px-2 sm:px-3 py-1 rounded text-green-700 font-semibold">
                                    {config?.rounds || 3}
                                </span>
                            </div>
                        </div>
                    ...
                    {/* Observaciones - Altura adaptativa */}
                    <section className="bg-white rounded-lg shadow p-3 sm:p-4 flex-1">
                        <h2 className="text-md sm:text-lg font-semibold text-[#00398A] mb-2">
                            Observaciones
                        </h2>
                        <textarea
                            ...
                            className="w-full h-[16rem] md:h-[calc(100%-4rem)] p-2 sm:p-3 border rounded-lg resize-none text-sm sm:text-base"
                ...
                <div className="bg-white rounded-lg shadow p-3 sm:p-4 flex flex-col h-full">
                    <h2 className="text-md sm:text-lg font-semibold text-[#00398A] mb-2 sm:mb-4">
                    ...
                    <div className="space-y-2 sm:space-y-3 flex-1 overflow-y-auto">
                        ...
                        <div className="mt-3 pt-3 border-t border-gray-200">
                            <h3 className="text-sm sm:text-md font-semibold text-[#00398A] mb-2">
                                Promedio por ronda
                            </h3>
                            <StatItem 
                                label="Aciertos por ronda" 
                                value={(stats?.num_aciertos / (stats?.totalRounds || 1)).toFixed(1)} 
                                highlightPositive={true}
                            />
                            <StatItem 
            ...
            {/* Botones - Mejorada la responsividad */}
            <div className="p-3 sm:p-4 bg-white flex flex-col sm:flex-row justify-center gap-3 sm:gap-4 mt-auto">
                <button
                    onClick={handleFinishSession}
                    disabled={loading}
                    className="px-4 sm:px-6 py-2 bg-[#00398A] text-white rounded-lg hover:bg-[#002d6f] transition-colors disabled:opacity-50 text-sm sm:text-base w-full sm:w-auto"
                >
                ...
                <button
                    ...
                    disabled={loading}
                    className="px-4 sm:px-6 py-2 bg-[#00A8E3] text-white rounded-lg hover:bg-[#0096cc] transition-colors disabled:opacity-50 text-sm sm:text-base w-full sm:w-auto"
                >
                    Repetir Sesión
                </button>
            </div>
...
// Componente para mostrar elementos de estadísticas con responsividad mejorada
const StatItem = ({ label, value, highlightPositive = false, highlightNegative = false }) => (
    <div className="flex justify-between items-center py-1 sm:py-1.5">
        ...
        <span className={`text-sm sm:text-base font-medium ${highlightPositive ? 'text-green-600' : ''} ${highlightNegative ? 'text-red-600' : ''}`}>
            {value}
        </span>
    </div>
);

export default ForestEnd;