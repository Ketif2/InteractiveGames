...
const PuzzleEnd = () => {
  ...
  if (!stats || !config) {
    return (
      <div className="min-h-[calc(100vh-10rem)] flex flex-col items-center justify-center bg-gray-50 p-4">
        <AlertTriangle className="w-16 h-16 text-yellow-500 mb-4" />
        <h2 className="text-xl font-semibold text-gray-800 mb-2 text-center">No se encontraron datos del juego</h2>
        <p className="text-gray-600 mb-6 text-center">No se pudieron cargar las estadísticas o la configuración del juego.</p>
        <button
  ...
  return (
    <div className="min-h-[calc(100vh-10rem)] flex flex-col bg-gray-50">
      ...
      <div className="bg-[#00398A] text-white py-2 px-4 sm:px-6">
        <h1 className="text-lg sm:text-xl font-semibold">Resultados del Rompecabezas</h1>
      </div>

      {/* Contenido principal */}
      <div className="flex-1 p-3 sm:p-4 grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4 min-h-0">
        {/* Columna izquierda */}
        <div className="flex flex-col gap-3 sm:gap-4 h-full">
          {/* Configuración */}
          <section className="bg-white rounded-lg shadow p-3 sm:p-4">
            <h2 className="text-base sm:text-lg font-semibold text-[#00398A] mb-2">
              Configuración Sesión
            </h2>
            <div className="space-y-2">
              <div className="flex justify-between items-center">
                <span className="text-sm sm:text-base text-gray-600">Dificultad:</span>
                <span className="bg-blue-100 px-2 sm:px-3 py-1 rounded text-sm sm:text-base">{config.difficulty}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm sm:text-base text-gray-600">Número de piezas:</span>
                <span className="bg-blue-100 px-2 sm:px-3 py-1 rounded text-sm sm:text-base">
                  {config.gridSize === '4' ? '16 (4x4)' : 
                  config.gridSize === '5' ? '25 (5x5)' : '36 (6x6)'}
                </span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-sm sm:text-base text-gray-600">Rompecabezas:</span>
                <span className="bg-blue-100 px-2 sm:px-3 py-1 rounded text-sm sm:text-base">
          ...
          <section className="bg-white rounded-lg shadow p-3 sm:p-4 flex-1">
            ...
            <textarea
              ...
              className="resize-none w-full h-[150px] md:h-[calc(100%-2rem)] p-2 sm:p-3 border rounded-lg overflow-y-auto text-sm sm:text-base"
        ...
        <div className="bg-white rounded-lg shadow p-3 sm:p-4 flex flex-col h-full">
          <h2 className="text-base sm:text-lg font-semibold text-[#00398A] mb-3 sm:mb-4">
            Estadísticas
          </h2>
      ...
      {/* Mensaje de error */}
      {error && (
        <div className="p-3 sm:p-4 bg-red-100 border border-red-200 text-red-700 text-center rounded mx-3 sm:mx-4 mb-3 sm:mb-4 text-sm sm:text-base">
          {error}
        </div>
      )}

      {/* Botones */}
      <div className="mt-2 bg-white p-3 sm:p-4 flex flex-col sm:flex-row justify-center gap-3 sm:gap-4">
        <button
...
// Componente de elementos de estadísticas con diseño responsive
